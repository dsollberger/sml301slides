...
For a binary classifier (two groups), we envision **random guessing** as analogous to a flip of a fair coin, so we say that the accuracy of a random guessing is 50 percent.
:::

::: {.callout-tip}
## Majority Classifier

A quick way to create a baseline for a classification metric is to envision the proportion for the most frequently observed group in the target class.
:::

```{r}
#| echo: false
penguin_class_df |>
  tabyl(chinstrap_bool)
```
$$\text{baseline accuracy: } \frac{265}{265 + 68} \approx 0.7958$$
...
::: {.column width="50%"}
**Binary** classification of $y \in \{0,1\}$
...
::: {.column width="40%"}
![logistic regression curve](logistic_regression_idea.png)

* image source: [Vijay Kanade](https://www.spiceworks.com/tech/artificial-intelligence/articles/what-is-logistic-regression/)
:::
...
```{r}
#| echo: false
logistic_model <- stats::glm(chinstrap_bool ~ flipper_length_mm,
...
x <- seq(round(min(penguin_class_df$flipper_length_mm)),
         round(max(penguin_class_df$flipper_length_mm)))
y <- 1 / (1 + exp(-beta_0 - beta_1*x))
log_reg_df <- data.frame(x,y)
```

```{r}
#| echo: false
penguin_class_df |>
  ggplot(aes(x = flipper_length_mm, y = as.integer(chinstrap_bool)-1)) +
  geom_point(aes(color = chinstrap_bool)) +
  ...
  labs(title = "Logistic Regression",
       subtitle = "One predictor variable",
       caption = "SML 301",
       x = "flipper length (mm)",
       y = "class (1: chinstrap, 0: not chinstrap)") +
  scale_color_manual(values = c("black", "#c65ccc")) +
  theme_minimal() +
...
* image source: [Lory Seraydarian](https://plat.ai/blog/confusion-matrix-in-machine-learning/)
...
* [accuracy](https://en.wikipedia.org/wiki/Sensitivity_and_specificity): 0.5285
* [precision](https://en.wikipedia.org/wiki/Sensitivity_and_specificity): 0.2595
* [recall](https://en.wikipedia.org/wiki/Sensitivity_and_specificity): 0.7059
:::
...
$$\text{accuracy} = \frac{TP}{TP + FN  + FP + TN}$$
...
* Improving Language Understanding by Generative Pre-Training (2018) aka GPT, Radford, Narasimhan, Salimans, and Sutskever
...
* XGBoost: A Scalable Tree Boosting System (2016), Chen and Guestrin
...
penguins <- penguins |>
  mutate(above_average_weight = ifelse(body_mass_g > mean(body_mass_g, na.rm = TRUE), 1, 0)) |>
  mutate(above_average_weight = factor(above_average_weight))
...
Naive Bayes is a **multiclass** classification tool.
:::
...
# Ethics History: Government Data

:::: {.columns}

::: {.column width="60%"}
"Governments and other institutions began collecting data on death, crime, and suicide in fits and starts. Most of them held this data closely; many even considered them secrets of state. Quetelet sought to obtain these numbers and then to publish them. He drew upon a broad European network of scientists to coax numbers from administrators and then published scores in his own journal. In the age of the internet, journal publication may seem ridiculously slow—but it was a radical transformation in the public availability and circulation of data."

Wiggins, Chris; Jones, Matthew L.. How Data Happened: A History from the Age of Reason to the Age of Algorithms (p. 25). W. W. Norton & Company. Kindle Edition. 	
:::
...
* [What is a Confusion Matrix](https://plat.ai/blog/confusion-matrix-in-machine-learning/) by Lory Seraydarian
* [What Is Logistic Regression?](https://www.spiceworks.com/tech/artificial-intelligence/articles/what-is-logistic-regression/) by Vijay Kanade
